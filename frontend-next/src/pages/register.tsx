...
export default function Register() {
  ...
  const onSubmit = async (data: RegisterFormData) => {
    ...
    try {
      ...
      const response = await axios.post(
        'https://obliged-wandis-aditech-18661c81.koyeb.app/api/auth/register', 
        registerData,
        {
          headers: {
  ...
  return (
    <>
      <Head>
        <title>Register | HR Recruitment System</title>
      </Head>
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-sky-50 to-white py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-md w-full space-y-8 bg-white p-8 rounded-xl shadow-lg transform transition-all duration-200 hover:shadow-xl">
          <div className="text-center">
            ...
            <p className="text-sm text-gray-600">Join HR Recruitment today</p>
          </div>
          {error && (
          ...
          <form className="mt-8 space-y-6" onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="text-center text-sm">
              ...
              <Link href="/login" className="font-medium text-sky-600 hover:text-sky-500 transition-colors duration-200">Sign in here</Link>
            </div>
          </form>
        </div>
      </div>
